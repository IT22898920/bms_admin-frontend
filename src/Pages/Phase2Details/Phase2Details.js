...
import "../Phase1 Details/Phase1Details.css";
...
const Phase2Details = () => {
  ...
  const handleNextStage = async () => {
    try {
      const response = await axios.put(
        `${process.env.REACT_APP_BACKEND_URL}/api/document/next-stage/${id}`,
        {},
        { withCredentials: true }
      );
      message.success(response.data.message);
        ...
        <div className="details-section">
          ...
          <div className="action-buttons">
            ...
            {document?.timelineStatus === "Done" ? (
              ...
              <button className="btn btn-next" onClick={handleNextStage}>
                Next
              </button>
            )}
          </div>
